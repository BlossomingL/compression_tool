...
def create_thinning_recipe_filters(sgraph, model, zeros_mask_dict):
    ...
    """
    def handle_layer(layer_name, param_name, num_nnz_filters, num_nnz_filters_next):
        ...
        ############################################################################################################
        if param_name.find('branch') != -1:
            if param_name.find('branch_proj') != -1:
                successors = []
            else:
        ...
        for successor in successors:
            if isinstance(layers[successor], torch.nn.modules.Conv2d):
                new_indices = indices
                ############################################################################################################
                if param_name.find('branch_main.5') != -1:
                    new_indices = torch.tensor(np.random.randint(0, num_filters, size=num_nnz_filters_next))
                ############################################################################################################
                handle_conv_successor(thinning_recipe, layers, successor, num_nnz_filters_next, new_indices)
    ...
    def handle_conv_successor(thinning_recipe, layers, successor, num_nnz_filters, indices):
        ...
        if layers[successor].groups == 1:
            ...
            handle_layer(successor, param_name, num_nnz_filters, num_nnz_filters)
        else:
            raise ValueError("Distiller thinning code currently does not handle this conv.groups configuration")
    ...
    def handle_prelu_layers(layers, layer_name, num_nnz_filters, indices):
         ...
         if layer_name.find('branch_main.4') == -1 and len(prelu_layers) >= 1 and prelu_layers[0] != 'conv_6_sep.prelu':
    ...
    for layer_name, param_name, param in sgraph.named_params_layers():
        # if param_name == 'features.0.branch_main.5.weight':
        #    print(param_name)
        ...
        # successors = sgraph.successors_f(layer_name, ['Conv', 'Gemm'])
        # print(layre_name, successors)
        #######################################################################################
        num_nnz_filters_next = num_nnz_filters
        
        if param_name.find('branch_main.5') != -1:
            param_name_split = param_name.split('.')
            param_name_split[2] = 'branch_proj'
            param_name_split[3] = '2'
            parallel_param_name = '.'.join(param_name_split)
            flag = False
            for layer_name_tmp, param_name_tmp, param_tmp in sgraph.named_params_layers():
                if param_name_tmp == parallel_param_name:
                    parallel_param = param_tmp
                    flag = True
                    break
            if param_name == 'features.2.branch_main.5.weight':
                print(param_name)
            if flag:
                parallel_filter_view = parallel_param.view(parallel_param.size(0), -1)
                parallel_num_filters = parallel_filter_view.size()[0]
                parallel_nonzero_filters = torch.nonzero(parallel_filter_view.abs().sum(dim=1))
                parallel_num_nnz_filters = parallel_nonzero_filters.nelement()
                num_nnz_filters_next = (parallel_num_nnz_filters + num_nnz_filters) // 2
                # print(parallel_num_nnz_filters, num_nnz_filters)
                # print(param_name, parallel_param_name, num_nnz_filters_next)
            else:
                param_name_split[2] = 'branch_main'
                param_name_split[3] = '0'
                first_param_name = '.'.join(param_name_split)

                parallel_num_nnz_filters = 0
                for param_name_tmp, param_directives in thinning_recipe.parameters.items():
                    if param_name_tmp == first_param_name:
                        for directive in param_directives:
                            if directive[0] == 1:
                                parallel_num_nnz_filters = len(directive[1])
                                break

                if parallel_num_nnz_filters == 0:
                    for layer_name_tmp, param_name_tmp, param_tmp in sgraph.named_params_layers():
                        if param_name_tmp == first_param_name:
                            # parallel_filter_view = param_tmp.view(param_tmp.size(0), -1)
                            # parallel_num_filters = parallel_filter_view.size()[0]
                            # parallel_nonzero_filters = torch.nonzero(parallel_filter_view.abs().sum(dim=1))
                            # parallel_num_nnz_filters = parallel_nonzero_filters.nelement()
                            parallel_num_nnz_filters = param_tmp.size()[1]
                            break
                num_nnz_filters_next = (parallel_num_nnz_filters + num_nnz_filters) // 2
        #######################################################################################
        handle_layer(layer_name, param_name, num_nnz_filters, num_nnz_filters_next)
    return thinning_recipe
...
def execute_thinning_recipe(model, zeros_mask_dict, recipe, optimizer, loaded_from_file=False):
    ...
    with torch.no_grad():
        for param_name, param_directives in recipe.parameters.items():
            ...
            ####################################################################################################################
            #if param_name.find('branch') != -1:
            #    if len(param_directives) == 3:
            #        first_directive = param_directives.pop(0)
            #        first_len = first_directive[1].nelement()
            #       second_directive = param_directives.pop(0)
            #        second_len = second_directive[1].nelement()
            #        new_len = (first_len + second_len) // 2
            #        new_param_directives = []
            #        new_param_directives.append(1)
            #        new_param_directives.append(torch.split(torch.cat((first_directive[1], second_directive[1])), new_len)[1])
            #        param_directives.append(new_param_directives)
            ####################################################################################################################
            for directive in param_directives:
                dim = directive[0]
