...
class PruningPolicy(ScheduledTrainingPolicy):
    ...
    def on_epoch_begin(self, model, zeros_mask_dict, meta, fpgm=False, HRank=False, conv_index={}, **kwargs):
        ...
        for param_name, param in model.named_parameters():
            if not is_initialized:
                ...
                if not self.skip_first_minibatch:
                    self.pruner.set_param_mask(param, param_name, zeros_mask_dict, meta, fpgm, HRank, conv_index)
            else:
                self.pruner.set_param_mask(param, param_name, zeros_mask_dict, meta, fpgm, HRank, conv_index)
