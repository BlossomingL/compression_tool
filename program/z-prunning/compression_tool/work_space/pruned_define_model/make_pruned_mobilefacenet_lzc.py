...
class MobileFaceNet(Module):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7), p=1, drop_p=0.75, num_classes=4, input_channel=3, fl=False):
        super(MobileFaceNet, self).__init__()
        # Focal Loss
        self.fl = fl
        self.embedding_size = embedding_size

        self.conv1 = Conv_block(input_channel, keep[0], kernel=(3, 3), stride=(2, 2), padding=(1, 1))
...
if __name__ == '__main__':
    ...
    model = MobileFaceNet(keep_dict['1.8M'], 128, (5, 5))

    x = torch.rand([2, 3, 80, 80])
    model.load_state_dict(torch.load('/home/user1/linx/program/LightFaceNet/work_space/models/pruned_model'
                                     '/model_mobilefacenet_lzc.pt'))
    model.cuda()
    x = x.to('cuda')
    out = model(x)
    print(out)
